...
public class PrizesController extends BaseController
{
    ...
    public AjaxResult getInfo(@PathVariable("maxAmount") BigDecimal maxAmount)
    {
        return success(prizesService.selectPrizesByMaxAmount(maxAmount));
    }
	...
	@DeleteMapping("/{maxAmounts}")
    public AjaxResult remove(@PathVariable BigDecimal[] maxAmounts)
    {
        return toAjax(prizesService.deletePrizesByMaxAmounts(maxAmounts));
    }
}
