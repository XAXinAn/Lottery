...
public class WinTransactionsServiceImpl implements IWinTransactionsService
{
    ...
    public List<Transactions> lottery() {
        ...
        if (time == 1) {
            ...
            result.clear();
            int totalOrgNoNum = transactionsMapper.getTotalOrgNoNum();
            int restOrgNoNum = totalOrgNoNum - 50;
            for (Prizes prize : prizesList) {
                if(prize == prizesList.get(0)){
                    List<BigDecimal> billAmounts = new ArrayList<>();
                    List<Transactions> temp = new ArrayList<>();
                    ...
                    do {
                        ...
                        int cnt = 0;
                        Transactions transaction = null;
                        for (int i = 0; i < transactionsList.size() && cnt < restOrgNoNum; i++) {
                            Collections.shuffle(shuffledList);
                            transaction = shuffledList.get(0);
                            boolean exists = false;
                            for (Transactions t : temp) {
                        ...
                        maxBillAmountTransaction = temp.stream().max(Comparator.comparing(Transactions::getBillAmt)).get();
                    } while (sum < prize.getBudget().doubleValue() * (1.0*restOrgNoNum / prize.getNum().doubleValue()) * (0.95 - looptime * 0.001) || sum >= prize.getBudget().doubleValue() * (1.0*restOrgNoNum / prize.getNum().doubleValue()) * (1 + looptime * 0.001));
                    result.addAll(temp);
                    finalResult.addAll(temp);
                    ...
                    } while (lastsum + sum < prize.getBudget().doubleValue() * (0.95 - looptime * 0.001) || lastsum + sum >= prize.getBudget().doubleValue() * (1 + looptime * 0.001) || (prize == prizesList.get(prizesList.size() - 1) && org1MaxWinMapper.getOrg1Status(maxBillAmountTransaction.getOrg1Code()) == 1));
                    result.addAll(temp);
                    finalResult.addAll(temp);
                    if(prize == prizesList.get(prizesList.size() - 1)){
                        org1MaxWinMapper.updateOrg1MaxWinStatusTo1(maxBillAmountTransaction.getOrg1Code());
                    ...
                    List<Transactions> temp = new ArrayList<>();
                    //取出金额区间内的交易记录
                    List<Transactions> transactionsList = transactionsMapper.selectTransactionsByAmountRangeWithLastWin(prize,time-1);
                    Transactions maxBillAmountTransaction = null;
                    //随机打乱交易记录
                    long looptime = 0;
                    do {
                        ...
                        maxBillAmountTransaction = temp.stream().max(Comparator.comparing(Transactions::getBillAmt)).get();
                    } while (sum < prize.getBudget().doubleValue() * (0.95 - looptime * 0.01) || sum >= prize.getBudget().doubleValue() * (1 + looptime * 0.01) || (prize == prizesList.get(prizesList.size() - 1) && org1MaxWinMapper.getOrg1Status(maxBillAmountTransaction.getOrg1Code()) == 1) );
                    result.addAll(temp);
                    finalResult.addAll(temp);
                    if(prize == prizesList.get(prizesList.size() - 1)){
                        org1MaxWinMapper.updateOrg1MaxWinStatusTo1(maxBillAmountTransaction.getOrg1Code());
